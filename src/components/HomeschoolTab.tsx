...
function StudentTile({ student, onClick, selected }: { student: StudentData; onClick: () => void; selected?: boolean }) {
  const c = COLOR_MAP[student.color] || COLOR_MAP.blue
  const doneCount = student.subjects.filter(s => s.status === 'done').length
    ...
    >
      ...
      <div className="space-y-0.5 mb-2">
        ...
        {student.subjects.length > 4 && (
          <div className="text-xs text-gray-400">+{student.subjects.length - 4} more</div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between text-xs">
        <span className={`font-medium ${c.text}`}>
          {doneCount}/{student.subjects.length} done
        </span>
        <span className="text-gray-500">
          {student.focus_sessions} focus
        </span>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border shadow-sm p-4">
        ...
        {familyBook ? (
          <div className="bg-teal-50 rounded-lg p-4">
            ...
            {familyBook.current_page && familyBook.total_pages && (
              <div className="mt-2">
                ...
                <div className="w-full bg-teal-200 rounded-full h-1.5">
                  <div
                    ...
                    style={{ width: `${(familyBook.current_page / familyBook.total_pages) * 100}%` }}
                  />
                </div>
              </div>
            )}
          ...
          <p className="text-sm text-gray-400 italic">No family read-aloud set</p>
        )}
      </div>
      ...
      {students.map(student => {
        const c = COLOR_MAP[student.color] || COLOR_MAP.blue
        return (
          <div key={student.id} className="bg-white rounded-lg border shadow-sm p-4">
            <h3 className="font-semibold text-gray-900 flex items-center gap-2 mb-3">
              <span>{student.mascot}</span>
              {student.name}'s Books
            </h3>
            {student.books.length === 0 ? (
              <p className="text-sm text-gray-400 italic">No books tracked</p>
            ) : (
              <div className="space-y-2">
                {student.books.map((book, i) => (
                  <div key={i} className={`${c.bg} rounded-lg px-3 py-2 flex items-center justify-between`}>
                    <div>
                      <p className="font-medium text-gray-800 text-sm">{book.title}</p>
                      <p className="text-xs text-gray-500">{book.author}</p>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      book.status === 'reading' ? `${c.light} ${c.text}` :
                      book.status === 'done' ? 'bg-green-100 text-green-700' :
...
function UnitsView({ units }: { units: UnitStudy[] }) {
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="space-y-3">
            {active.map(unit => (
              <div key={unit.id} className="bg-white rounded-lg border shadow-sm p-4">
                ...
                <p className="text-xs text-gray-400 mt-2">Started {unit.start_date}</p>
              </div>
            ))}
          </div>
        )}
      ...
      {completed.length > 0 && (
        <div>
          <h3 className="font-semibold text-gray-700 mb-3">Completed</h3>
          <div className="space-y-2">
            {completed.map(unit => (
              <div key={unit.id} className="bg-gray-50 rounded-lg border p-3 flex items-center justify-between">
                <div>
                  <h4 className="font-medium text-gray-700">{unit.title}</h4>
                  <p className="text-xs text-gray-500">{unit.start_date} &ndash; {unit.end_date}</p>
                </div>
