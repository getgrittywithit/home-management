...
import { FlaskConical, Send, SkipForward, Loader2, Star, Trophy, Lightbulb } from 'lucide-react'
...
export default function ScienceBuddy({ kidName, onStarsEarned }: ScienceBuddyProps) {
  ...
  const loadNextSkill = async () => {
    ...
    setShowHint(false)
    try {
      const res = await fetch(`/api/learning-engine?action=next_science_skill&kid_name=${kidName.toLowerCase()}`)
      const data = await res.json()
      setSkill(data)
      setSessionId(data.session_id || `elar-${Date.now()}`)

      // Load a passage for this skill, avoiding already-seen passages
      const pRes = await fetch(`/api/learning-engine?action=science_placement_passages&skill_id=${data.skill_id}&level=2nd-3rd`)
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg border shadow-sm p-8 text-center">
        ...
        <p className="text-gray-500">Loading your next science challenge...</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-500 to-fuchsia-500 text-white p-5 rounded-xl">
        <div className="flex items-center gap-2 mb-1">
          <FlaskConical className="w-6 h-6" />
          <h2 className="text-xl font-bold">Science Buddy</h2>
        </div>
        {skill && (
          <div className="mt-2">
            <p className="text-purple-100 text-sm">Current Focus: {skill.skill_name}</p>
            ...
            <p className="text-purple-200 text-xs mt-1">{skill.reason}</p>
          </div>
        )}
      </div>
        ...
        <div className="bg-white rounded-lg border shadow-sm p-5">
          ...
          <div className="border-t pt-4">
            ...
            <textarea
              ...
              rows={4}
              disabled={!!feedback}
              className="w-full border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 resize-none disabled:bg-gray-50"
            />
            {!feedback && (
      ...
      {feedback && (() => {
        ...
        <div className={`rounded-lg border shadow-sm p-5 ${
          ...
          <button onClick={handleNext}
            className="w-full mt-4 bg-purple-600 text-white py-2.5 rounded-lg font-medium hover:bg-purple-700">
            Next Question
          </button>
        </div>
        )
