...
import { Globe2, Send, SkipForward, Loader2, Star, Trophy, Lightbulb } from 'lucide-react'

interface HistoryBuddyProps {
  kidName: string
  onStarsEarned?: (count: number) => void
}

export default function HistoryBuddy({ kidName, onStarsEarned }: HistoryBuddyProps) {
  ...
  const loadNextSkill = async () => {
    ...
    setShowHint(false)
    try {
      const res = await fetch(`/api/learning-engine?action=next_history_skill&kid_name=${kidName.toLowerCase()}`)
      const data = await res.json()
      setSkill(data)
      setSessionId(data.session_id || `elar-${Date.now()}`)

      // Load a passage for this skill, avoiding already-seen passages
      const pRes = await fetch(`/api/learning-engine?action=history_placement_passages&skill_id=${data.skill_id}&level=2nd-3rd`)
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg border shadow-sm p-8 text-center">
        ...
        <p className="text-gray-500">Loading your next history challenge...</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-amber-500 to-orange-500 text-white p-5 rounded-xl">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="text-xl font-bold">History Buddy</h2>
        </div>
        {skill && (
          <div className="mt-2">
            <p className="text-amber-100 text-sm">Current Focus: {skill.skill_name}</p>
            <div className="flex items-center gap-2 mt-1">
              <div className="flex-1 bg-amber-400/30 rounded-full h-2">
            ...
            <p className="text-amber-200 text-xs mt-1">{skill.reason}</p>
          </div>
        )}
      </div>
        ...
        <div className="bg-white rounded-lg border shadow-sm p-5">
          ...
          <div className="border-t pt-4">
            ...
            <textarea
              ...
              rows={4}
              disabled={!!feedback}
              className="w-full border rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 resize-none disabled:bg-gray-50"
            />
            {!feedback && (
              <div className="space-y-2 mt-3">
                ...
                <div className="flex gap-2">
                  <button onClick={handleSubmit} disabled={submitting || !response.trim()}
                    className="flex-1 bg-amber-600 text-white py-2.5 rounded-lg font-medium hover:bg-amber-700 disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      {feedback && (() => {
        ...
        <div className={`rounded-lg border shadow-sm p-5 ${
          ...
          <button onClick={handleNext}
            className="w-full mt-4 bg-amber-600 text-white py-2.5 rounded-lg font-medium hover:bg-amber-700">
            Next Question
          </button>
        </div>
        )
