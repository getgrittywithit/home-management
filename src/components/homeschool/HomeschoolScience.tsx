...
import ScienceBuddy from './ScienceBuddy'
import JourneyMap from './JourneyMap'
import PlacementQuiz from './PlacementQuiz'
import { Sparkles } from 'lucide-react'

type SciView = 'science-buddy' | 'journey-map'

export default function HomeschoolScience() {
  const [view, setView] = useState<SciView>('science-buddy')
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
        <button onClick={() => setView('science-buddy')}
          className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
            view === 'science-buddy' ? 'bg-white text-purple-700 shadow-sm' : 'text-gray-500'
          }`}>Science Buddy</button>
        <button onClick={() => setView('journey-map')}
          className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
            view === 'journey-map' ? 'bg-white text-purple-700 shadow-sm' : 'text-gray-500'
          }`}>Journey Map</button>
      </div>
          ...
          {[...STUDENT_DEFAULTS, { id: 'zoey', name: 'Zoey', grade: '9th', mascot: '🌟', mascotName: 'Star', color: 'pink' }, { id: 'kaylee', name: 'Kaylee', grade: '7th', mascot: '🎭', mascotName: 'Drama', color: 'rose' }].map(s => (
            <button key={s.id} onClick={() => setSelectedKid(s.name)}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap ${
                selectedKid === s.name ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}>
              {s.mascot} {s.name}
            </button>
          ))}
      ...
      {view === 'science-buddy' && <ScienceBuddy kidName={selectedKid} />}
      {view === 'journey-map' && <JourneyMap kidName={selectedKid} subject="science" />}
    </div>
  )
}
